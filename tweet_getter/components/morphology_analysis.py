...
class MorphAnalysis:
    ...
    def parse_text_to_words(self, text, exclude_dont_list=False):
        ...
        if exclude_dont_list:
            words = [
                parsed_in_line.split("\t")[0]
                for parsed_in_line in parsed_in_line_list
                if parsed_in_line.split("\t")[0] not in self.dont_list
            ]
        ...
        for extr in extr_dict.values():
            ...
            for i_str in range(int(parsed_text["length"])):
                ...
                if fragmented_str not in aggr_dict.keys():
                    ...
                    aggr_dict[fragmented_str] = {
                        "string": parsed_text[i_str]["string"],
                        "amount": 1,
                        "POS": parsed_text[i_str]["POS"],
                        "subPOS": parsed_text[i_str]["subPOS"],
                    }
                ### あるやつは数を増やす。
                else:
                    aggr_dict[fragmented_str]["amount"] = (
                        int(aggr_dict[fragmented_str]["amount"]) + 1
                    )
...
if __name__ == "__main__":
    ...
    for extr_path in extr_path_list:
        ...
        for extr in extr_dict.values():
            if extr["dancer_name"] != "29nikunikuniku":
                continue
            ...
            print(
                'in {}\'s "{}", length of molphed: {}, total counts: {}'.format(
                    extr["dancer_name"], text_to_parse, len(molphed_tweet), word_dist_series.sum()
                )
            )
            # if i_count > 3: break
        ...
        idf_series = pd.Series(
            [math.log(Ndf, math.e) + 1.0 for Ndf in Ndf_series], index=Ndf_series.index
        )
